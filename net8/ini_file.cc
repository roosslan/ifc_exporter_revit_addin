...
namespace ifc_exporter {
        ...
        array<string>^ ini_simple::read_section(const string section) {
	        ...
	        IntPtr p_returned_string = Marshal::AllocCoTaskMem((int)max_buffer * sizeof(char));
            int bytesReturned = GetPrivateProfileSectionW(section, p_returned_string, max_buffer, ext_path_);
            ...
            const string returned_string = Marshal::PtrToStringAuto(p_returned_string, bytesReturned - 1);
